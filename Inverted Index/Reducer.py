...
class Reducer(pb2_grpc_reducer.ReducerServicer):
  ...
  def parseFile(self,inputFile,invertedIndexDict):
    ...
    for line in fileLines:
      ...
      word = lineSplit[0]
      docsSet = set(lineSplit[1:])
      if word in invertedIndexDict.keys():
        invertedIndexDict[word] = invertedIndexDict[word] | docsSet
      else:
  ...
  def GetInputForReducerOperations(self, request, context):
    ...
    reducerThread = Thread(target=self.invertedIndex,args = [inputFileList,outputFileName])
    reducerThread.start()
